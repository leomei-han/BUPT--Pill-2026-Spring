...
module bottle_fsm(
    ...
    reg [7:0]  acc_cur;          // 本瓶药片 00-99 （2 位 BCD，最大计到装入量上限 99 的前一颗即换瓶）
    ...
    // 用于瓶内药片数和已完成瓶数：
    //   acc_cur 最大计到 98（装入量上限 99 的前一颗即满瓶清零），十位 ≤ 9，行为与纯 BCD 完全一致
    ...
    //   dec8_bcd(8'h99)=8'h98（装入量），dec8_bcd(8'hA0)=8'h99（瓶数 100→99）
